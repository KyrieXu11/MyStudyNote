...
4. 解决循环依赖问题；可参考[Spring管理循环依赖](https://juejin.im/post/6844904122160775176)
   ...
   1. 首先A提前将自己曝光到singletonFactories中去
   2. 然后发现自己依赖对象B，就去获取B的对象，发现对象B还没有被创建，所以创建B
   3. 创建B的时候将自己曝光到singletonFactories中去
   4. 发现自己依赖对象A，就去获取对象，从singletonFactories获取创建A的ObjectFactory，创建A，然后放到earlySingletonObjects，然后注入A到B中去
   5. 然后将B放到singletonObjects中去，最后将A放到singletonObjects中去，循环依赖解决完成。
...
轻量级锁：不存在锁竞争，没有线程堵塞。栈的局部变量表中有存放对象的引用，也可以理解为一种指针。所以在检测到是轻量级锁的情况之后，就用CAS来改变轻量级锁中Mark Word中指向栈帧的锁记录的指针，把锁对象的HashCode复制到自己新建的用于存储锁的记录空间，如果成功了就表示当前的线程获得到了锁，如果失败了就一直自旋等待获取锁的线程释放锁，但是不会一直自旋，如果自旋了很久就升级成重量级锁。
...
NIO：同步非阻塞IO，使用多路复用IO方式（多个IO共同使用一个线程），来保证并发，多路复用有select、poll、epoll，区别见[多路复用IO的对比](#Redis的多路复用IO模型)

AIO：异步非阻塞IO。基于事件和回调机制实现的，应用操作会返回不会阻塞在方法中，当完成时，操作系统通知相关线程执行完毕，进行后续操作，和NIO的区别就是：NIO当一个线程完成IO之后，这个线程会继续去进行IO操作
...
```java
// com.spi.SPIService 存放在META-INF/services下，内容是
...
通过线程上下文加载器来进行打破，这个加载器可以在线程创建的时候设置，如果没有设置的话那么就会从父线程继承一个，那么这个加载器就是应用程序加载器，上面的SPI技术是父加载器请求子加载器的行为，已经违反了双亲委派模型
