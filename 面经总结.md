...
参考[ThreadLocal](https://zhuanlan.zhihu.com/p/102571059)
...
如果创建ThreadLocal的线程一直在运行的话，由于Entry是实现了弱引用的接口的，所以在发生内存收集的时候会将Entry给回收，但是value还是保持着强引用，所以value是不会被回收的，从而导致内存泄漏，但是要避免内存泄漏的话久需要手动删除value，也就是调用`Remove()`方法来清除value值
...
查询浏览器的缓存DNS，查询hosts文件，没有的话就查询本地的DNS服务器，查询本地的DNS服务器是递归查询，如果本地的DNS中查不到，就找根域名的DNS服务器，这个查询方式是迭代查询，根域名的DNS根据域名返回给本地DNS他应该找哪个DNS服务器c去找，c返回给本地DNS服务器权威DNS服务器的地址，然后本地的DNS又去找权威DNS服务器，才得到IP地址
