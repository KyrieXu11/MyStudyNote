...
+ survivor空间中相同年龄的对象大小综合大于Survivor空间的一半，则年龄大于等于该年龄的对象可进入老年区
...
1. 慢开始：TCP在开始发送数据的时候首先会发送少量的数据探探路，然后再决定用什么样的速率传输。而速率取决于拥塞窗口的大小，开始拥塞窗口为1，**每次收到ACK报文的时候，就让拥塞窗口大小变为原来的2倍。当窗口值=慢开始门限初始值（一般为16）**，就不再是2倍增长，而是改为每次增加1，直到网络拥塞，此时采用拥塞避免的算法
2. 拥塞避免：让新的慢开始门限值变为原来的一半，将拥塞窗口设置为1，依次往复就会让网络传输的拥塞程度大大降低。
